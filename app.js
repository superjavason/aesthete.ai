...
function renderGrowthInsight() {
  ...
  const message =
    totalQuestCompletions === 0
      ? "You are at the awakening stage. The first win is not intensity but consistency: complete one quest, then record one reflection."
      : `Your strongest muscle is ${strongest}. Your current stretch zone is ${weakest}. Keep the strong side alive, but use the next few sessions to balance the system so taste becomes an identity, not a trick.`;
